...
if ($_SESSION['role_action']['privilege_history']['view'] != 1) {
...
$sql_transaction = 'SELECT
						...
						member_transaction_h.meth_Platform AS platform
...
if (!$rs_trans) {
	...
	while($axRow = $rs_trans->FetchRow(DBI_ASSOC)) {
		...
		$view = '<button type="button" class="btn btn-default btn-sm" data-toggle="modal" data-target="#View'.$n.'"><span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span></button>
							    ...
							    <table width="100%" class="table table-striped table-bordered myPopup">
							        <thead>
										<tr class="th_table">
											...
											<th style="width:20%;text-align:ceter"><b>Number of Use</b></th>
											<th style="width:10%;text-align:ceter"><b>Number of Delete</b></th>
										</tr>
									</thead>
									<tbody>
										'.$data_view.'
...
$oTmp->assign('is_menu', 'is_member_insight');

$oTmp->assign('content_file','member_insights/member_use.htm');
