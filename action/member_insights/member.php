...
if ($_SESSION['role_action']['member']['view'] != 1) {
...
for($k=0 ; $k<count($_POST["card_id"]) ; $k++){

	if(trim($_POST["card_id"][$k]) != ""){

		if ($_POST["card_id"][$k]==0) { $card_id = 0; } 

		else {

			if ($k==count($_POST["card_id"])-1) { $card_id .= $_POST["card_id"][$k]; } 
			else { $card_id .= $_POST["card_id"][$k].","; }
		}
	}
}

if ($card_id=="" || $card_id==0) { $card_id = ""; }
else { $_SESSION['export_card'] = $card_id; }
...
if ($StartDate && $EndDate) {
	...
	$where_date = ' AND (SELECT MIN(b.date_create) FROM mb_member_register b WHERE b.member_id=a.member_id GROUP BY b.member_id) BETWEEN "'.$StartDate.'" AND "'.$EndDate1.'" ';
...
if($Act=='xls' || $Act=='csv'){
	...
	$master_field .= '<tr class="th_table">
				        ...
				        <th rowspan="2" style="text-align:center">First Register Date</th>
						<th rowspan="2" style="text-align:center">Last Updated Date</th>';
	...
	$sql_member = 'SELECT mb_member.* FROM mb_member';
	...
	if ($check_member) {
		...
		while ($member = $oRes_member->FetchRow(DBI_ASSOC)){
			...
			$data_table .= '<tr>
							  	...
							  	<td>'.DateTime($register_date).'</td>
							  	<td>'.DateTime($member['date_update']).'</td>';
...
if($_SESSION['user_type_id_ses']>1){
	...
	$sql_card ='SELECT card_id, name FROM mi_card WHERE brand_id='.$_SESSION['user_brand_id'].' AND flag_del!=1 ORDER BY name';
	...
	if ($card_id==0) {	$selected = "selected";	}
	else {	$selected = "";	}

	$select_card .= '<option value="0" '.$selected.'>All</option>';

	$selected = "";

	while ($axRow = $oRes_card->FetchRow(DBI_ASSOC)){

		for($j=0 ; $j<count($_POST["card_id"]) ; $j++){

			if ($axRow['card_id']==$_POST["card_id"][$j]) {	$selected = "selected";	}
		}

		$select_card .= '<option value="'.$axRow['card_id'].'" '.$selected.'>'.$axRow['name'].'</option>';

		$selected = "";
	}

	$oTmp->assign('select_card', $select_card);
}
...
$oTmp->assign('content_file', 'member_insights/member.htm');
