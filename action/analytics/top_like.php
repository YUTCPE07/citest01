...
if ($type != "Earn Attention") {
	...
	while ($axRow_card = $oRes_card->FetchRow(DBI_ASSOC)){
		...
		if ($type=="Privilege") {
			...
			$total_use = get_total_privilege_like($id,"",$card_id,"");

		} else if ($type=="Coupon" || $type=="Birthday Coupon") {

			$total_use = get_total_coupon_like($id,"",$card_id,"");

		} else {

			$total_use = get_total_activity_like($id,"",$card_id,"");
		}


		$table_like .= '<table class="myPopup">
							<tr>
		...
		while ($axRow_branch = $oRes_branch->FetchRow(DBI_ASSOC)) {
			...
			$table_like .= "<th style='text-align:center'>".$axRow_branch['txt']."</th>";
		...
		if($_SESSION['user_type_id_ses'] != 3) {
			...
			$table_like .= "<th style='text-align:center;'>รวม</th>";
		}

		$table_like .= "</tr></thead><tbody>";
		...
		if ($type=="Privilege") {
			...
			$trans_like = "member_privilege_trans.mepe_Like";
			...
			$trans_like = "member_coupon_trans.meco_Like";
			...
			$trans_like = "member_activity_trans.meac_Like";
		}
			...
			while ($axRow = $oRes_register->FetchRow(DBI_ASSOC)) {
				...
				for ($i=0; $i < $a; $i++) {
					...
					if ($type=="Privilege") {
						...
						$member_like = get_total_privilege_like($id,$branch_id[$i],$card_id,$axRow['member_id']);

					} else if ($type=="Coupon" || $type=="Birthday Coupon") {

						$member_like = get_total_coupon_like($id,$branch_id[$i],$card_id,$axRow['member_id']);

					} else {

						$member_like = get_total_activity_like($id,$branch_id[$i],$card_id,$axRow['member_id']);
					}

					$table_like .= "<td style='text-align:center'>".number_format($member_like)."</td>";
				}

				if($_SESSION['user_type_id_ses'] != 3) {

					$table_like .= "<td style='text-align:center;background-color:#F2F2F2'><b>".number_format($axRow['total'])."</b></td>";
				}
	...
	$total_use = get_total_earn_like($id,"","");
	...
	if($_SESSION['user_type_id_ses'] != 3) {
		...
		$table_like .= "<th style='text-align:center;width:10%'>รวม</th>";
	}
	...
	if ($check_use) {
		...
		while ($axRow = $oRes_use->FetchRow(DBI_ASSOC)) {
			...
			for ($i=0; $i < $a; $i++) {
				...
				$member_like = get_total_earn_like($id,$branch_id[$i],$axRow['member_id']);
			...
			if($_SESSION['user_type_id_ses'] != 3) {
				...
				$table_like .= "<td style='text-align:center;background-color:#F2F2F2'><b>".number_format($total_member)."</b></td>";
			}
...
$oTmp->assign('content_file', 'analytics/top_like.htm');
