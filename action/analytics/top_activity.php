...
$sql_card_activity = 'SELECT DISTINCT
						mi_card.*,
...
$oRes_card = $oDB->Query($sql_card_activity)or die(mysql_error());
...
while ($axRow_card = $oRes_card->FetchRow(DBI_ASSOC)){
	...
	if ($x > 1) { $table_activity .= '<hr>'; }
	else { $table_activity .= '<br>'; }
	...
	$sql_activity = 'SELECT acti_Name as txt, acti_ActivityID as id 
						FROM activity 
						WHERE bran_BrandID = "'.$brand_id.'" 
						AND acti_ActivityID = "'.$id.'" ';

	$oRes_branch = $oDB->Query($sql_branch)or die(mysql_error());

	$oRes_activity = $oDB->Query($sql_activity)or die(mysql_error());

	$total_use = get_total_activity_use($id,"",$card_id,"");


	$table_activity .= '<table class="myPopup">
							<tr>
								<td rowspan="4" width="200px" valign="top" style="text-align:center;">
									<a href="../card/card.php">'.$axRow_card['card_image'].'</td>
	...
	while ($axRow_branch = $oRes_branch->FetchRow(DBI_ASSOC)) {
		...
		$table_activity .= "<th style='text-align:center'>".$axRow_branch['txt']."</th>";
	...
	if($_SESSION['user_type_id_ses'] != 3) {
		...
		$table_activity .= "<th style='text-align:center;width:10%'>รวม</th>";
	}

	$table_activity .= "</tr></thead><tbody>";
	...
	$sql_register = 'SELECT DISTINCT
						...
						COUNT(member_activity_trans.meac_MemberActivityID) AS total
						...
						AND member_activity_trans.acti_ActivityID = "'.$id.'"
						GROUP BY member_activity_trans.memb_MemberID
						ORDER BY total DESC';
	...
	if ($check_regis) {
		...
		while ($axRow = $oRes_register->FetchRow(DBI_ASSOC)) {
			...
			for ($i=0; $i < $a; $i++) {
				...
				$member_acti = get_total_activity_use($id,$branch_id[$i],$card_id,$axRow['member_id']);

				$total_member += $member_acti;

				$table_activity .= "<td style='text-align:center'>".number_format($member_acti)."</td>";
			}

			if($_SESSION['user_type_id_ses'] != 3) {

				$table_activity .= "<td style='text-align:center;background-color:#F2F2F2'><b>".number_format($total_member)."</b></td>";
			}
...
$oTmp->assign('content_file', 'analytics/top_activity.htm');
