...
if ($StartDate && $EndDate) {
	...
	$where_date = ' AND hilight_coupon_buy.hcbu_CreatedDate BETWEEN "'.$StartDate.'" AND "'.$EndDate1.'" ';
	...
	$where_date = ' AND hilight_coupon_buy.hcbu_CreatedDate >= "'.$StartDate.'" ';
	...
	$where_date = ' AND hilight_coupon_buy.hcbu_CreatedDate <= "'.$EndDate1.'" ';
...
$sql_earn ='SELECT
					hilight_coupon_buy.hico_HilightCouponID,
					hilight_coupon_buy.hcbu_CreatedDate,
					...
					mi_token_type.name AS token_name,
					hilight_coupon_buy.*
					...
					ORDER BY hilight_coupon_buy.hcbu_CreatedDate DESC';
...
if($Act=='xls'){
	...
	$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue($chars++.$row_start, 'Buy Date')

				->setCellValue($chars++.$row_start, 'Buy Code')
				...
				->setCellValue($chars++.$row_start, 'Promotion')
	...
	while ($axRow = $oRes->FetchRow(DBI_ASSOC)) {
		...
		$objPHPExcel->getActiveSheet()->setCellValue($chars++ . $row_start, DateTime($axRow['hcbu_CreatedDate']));

		$objPHPExcel->getActiveSheet()->setCellValue($chars++ . $row_start, $axRow['hcbu_HilightCouponBuyID']);
	...
	if (!$rs_earn) {
		...
		while($axRow = $rs_earn->FetchRow(DBI_ASSOC)) {
			...
			# TABLE
		  	...
		  	$data_earn .= '<tr>
								<td>'.$earn_n++.'</td>
								<td style="text-align:center">'.$axRow['hcbu_HilightCouponBuyID'].'</td>
			...
			if ($_SESSION['user_type_id_ses']==1) {
				...
				$data_earn .= '<td >'.$axRow['token_name'].'</td>';
			}

			$data_earn .= '		<td style="text-align:right">'.number_format($axRow['hcbu_TotalAmount'],2).' ฿</td>
								<td style="text-align:center">'.$axRow['platform'].'</td>
								<td>'.DateTime($axRow['hcbu_CreatedDate']).'</td>
							</tr>' ;
		}
	}
}
...
$oTmp->assign('content_file','analytics/earn_buy.htm');
