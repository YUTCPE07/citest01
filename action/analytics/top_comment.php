...
if ($type != "Earn Attention") {
	...
	while ($axRow_card = $oRes_card->FetchRow(DBI_ASSOC)){
		...
		if ($x > 1) { $table_comment .= '<hr>'; }
		else { $table_comment .= '<br>'; }
		...
		if ($type=="Privilege") {
			...
			$total_use = get_total_privilege_comment($id,"",$card_id,"");

		} else if ($type=="Coupon" || $type=="Birthday Coupon") {

			$total_use = get_total_coupon_comment($id,"",$card_id,"");

		} else {

			$total_use = get_total_activity_comment($id,"",$card_id,"");
		}


		$table_comment .= '<table class="myPopup">
							<tr>
						...
						<br>
						<center>
	 						<span style="font-size:16px"><b>Total Review &nbsp; : &nbsp; '.number_format($total_use).' &nbsp; Times</span></b>
	 					</center>
	 					<br>
		...
		while ($axRow_branch = $oRes_branch->FetchRow(DBI_ASSOC)) {
			...
			$table_comment .= "<th style='text-align:center'>".$axRow_branch['txt']."</th>";
		...
		if($_SESSION['user_type_id_ses'] != 3) {
			...
			$table_comment .= "<th style='text-align:center;'>รวม</th>";
		}

		$table_comment .= "</tr></thead><tbody>";
		...
		if ($type=="Privilege") {
			...
			$trans_comment = "member_privilege_trans.mepe_Comment";
			...
			$trans_comment = "member_coupon_trans.meco_Comment";
			...
			$trans_comment = "member_activity_trans.meac_Comment";
		}
		...
		$sql_register = 'SELECT DISTINCT
							...
							COUNT(member_transaction_h.meth_MemberTransactionHID) AS total
							...
							AND member_transaction_h.meth_Deleted = ""
			...
			while ($axRow = $oRes_register->FetchRow(DBI_ASSOC)) {
				...
				for ($i=0; $i < $a; $i++) {
					...
					if ($type=="Privilege") {
						...
						$member_comment = get_total_privilege_comment($id,$branch_id[$i],$card_id,$axRow['member_id']);

					} else if ($type=="Coupon" || $type=="Birthday Coupon") {

						$member_comment = get_total_coupon_comment($id,$branch_id[$i],$card_id,$axRow['member_id']);

					} else {

						$member_comment = get_total_activity_comment($id,$branch_id[$i],$card_id,$axRow['member_id']);
					}

					$total_member += $member_comment;

					$table_comment .= "<td style='text-align:center'>".number_format($member_comment)."</td>";
				}

				if($_SESSION['user_type_id_ses'] != 3) {

					$table_comment .= "<td style='text-align:center;background-color:#F2F2F2'><b>".number_format($total_member)."</b></td>";
				}
	...
	$total_use = get_total_earn_comment($id,"","");
	...
	$table_comment .= "<table id='example' class='table table-bordered' style='background-color:white;'>
						<thead><tr class='th_table'>
						<th style='text-align:center;'>Member</th>
						<th style='text-align:center;'>Profile</th>";
	...
	if ($check_use) {
		...
		while ($axRow = $oRes_use->FetchRow(DBI_ASSOC)) {
			...
			for ($i=0; $i < $a; $i++) {
				...
				$member_like = get_total_earn_comment($id,$branch_id[$i],$axRow['member_id']);

				$total_member += $member_like;

				$table_comment .= "<td style='text-align:center'>".number_format($member_like)."</td>";
			}
...
$oTmp->assign('table_comment', $table_comment);

$oTmp->assign('data', $asData);

$oTmp->assign('is_menu', 'is_analytics');

$oTmp->assign('content_file', 'analytics/top_comment.htm');
