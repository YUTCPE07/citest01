...
if ($type != "Earn Attention") {
	...
	if ($type=="Privilege") {
		...
		$privilege_comment = 'mepe_Comment';
		...
		$privilege_comment = 'meco_Comment';
		...
		$privilege_comment = 'meac_Comment';
	}

	$sql_comment = "SELECT DISTINCT
						...
						mb_member.mobile,
						mb_member.email,
						".$table_trans.".".$privilege_comment." AS comment
	...
	$oRes_comment = $oDB->Query($sql_comment)or die(mysql_error());

	$check_comment = $oDB->QueryOne($sql_comment);
	...
	if ($check_comment) {
		...
		while ($axRow = $oRes_comment->FetchRow(DBI_ASSOC)) {
	...
	$sql_comment = "SELECT 
					...
					hilight_coupon_trans.hico_Comment AS comment,
...
$oTmp->assign('content_file', 'analytics/last_comment.htm');
