...
$sql_point = 'SELECT
					...
					reward_redeem_trans.rera_RewardQty_Stamp AS redeem_stamp,
...
$total_sql = "SELECT SUM(reward_redeem_trans.rera_RewardQty_Stamp)
					FROM reward_redeem_trans
...
$table_member = "<center><br>
					<span style='font-size:16px'><b>Total Stamp Use &nbsp; : &nbsp; ".number_format($total_use)."</span></b>
				<br></center>";
...
$table_member .= "<table id='example' class='table table-bordered' style='background-color:white;' >
					<thead>
						<tr class='th_table'>
							...
							<th style='text-align:center'><b>Stamp Use</b></th>
						</tr>
					</thead>";
...
if ($check_point) {
	...
	while ($axRow = $oRes_point->FetchRow(DBI_ASSOC)){
		...
		if($axRow['reward_image']!=''){
			...
			$axRow['reward_image'] = '<img src="../../upload/'.$axRow['rewa_ImagePath'].$axRow['reward_image'].'" width="50px" height="50px" class="image_border">';

		} else {

			$axRow['card_imreward_imageage'] = '<img src="../../images/400x400.png" width="50px" height="50px" class="image_border">';
		}
		...
		$table_member .= "<tr><td style='text-align:center'>".DateOnly($axRow['redeem_date'])."</td>
							<td style='text-align:center'>".$axRow['reward_image']."<br>
								<span style='font-size:11px'>".$axRow['reward_name']."</a></td>
							<td>".$axRow['redeem_name']."</td>
							<td>".$axRow['branch_name']."</td>
							<td style='text-align:center'>".$coll_image."<br>
								<span style='font-size:11px'>".$axRow['coty_Name']."</a></td>
							<td style='text-align:center'>".$auto_redeem."</td>
							<td style='text-align:center;background-color:#F2F2F2'><b>".number_format($axRow['redeem_stamp'])."</b></td>
						</tr>";
	}
...
$oTmp->assign('content_file', 'analytics/stamp_use.htm');
