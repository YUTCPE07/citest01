...
while ($axRow = $oRes->FetchRow(DBI_ASSOC)){
	...
	if ($axRow['coup_Motivation'] == 'Point') { 
		...
		$axRow['coup_Motivation'] = $point['mopp_Name'].' ('.number_format($point['mopp_UseAmount'],2).' ฿ / '.$point['mopp_PointQty'].' '.$icon.')';
		...
		$axRow['coup_Motivation'] = $stamp['mops_Name'].' (1 Times / '.$stamp['mops_StampQty'].' '.$icon.')';
	} 
...
$sql_card_coupon = 'SELECT DISTINCT
						mi_card.*,
...
$oRes_card = $oDB->Query($sql_card_coupon)or die(mysql_error());
...
while ($axRow_card = $oRes_card->FetchRow(DBI_ASSOC)){
	...
	if ($x > 1) { $table_coupon .= '<hr>'; }
	else { $table_coupon .= '<br>'; }
	...
	$total_use = get_total_coupon_use($id,"",$card_id,"");


	$table_coupon .= '<table class="myPopup">
							<tr>
	...
	while ($axRow_branch = $oRes_branch->FetchRow(DBI_ASSOC)) {
		...
		$table_coupon .= "<th style='text-align:center'>".$axRow_branch['txt']."</th>";
	...
	if($_SESSION['user_type_id_ses'] != 3) {
		...
		$table_coupon .= "<th style='text-align:center;width:10%'>รวม</th>";
	}

	$table_coupon .= "</tr></thead><tbody>";
	...
	$sql_register = 'SELECT DISTINCT
						...
						COUNT(member_coupon_trans.meco_MemberCouponID) AS total
						...
						AND member_coupon_trans.coup_CouponID = "'.$id.'"
						GROUP BY member_coupon_trans.memb_MemberID
						ORDER BY total DESC';
	...
	if ($check_regis) {
		...
		while ($axRow = $oRes_register->FetchRow(DBI_ASSOC)) {
			...
			for ($i=0; $i < $a; $i++) {
				...
				$member_coup = get_total_coupon_use($id,$branch_id[$i],$card_id,$axRow['member_id']);

				$total_member += $member_coup;

				$table_coupon .= "<td style='text-align:center'>".number_format($member_coup)."</td>";
			}

			if($_SESSION['user_type_id_ses'] != 3) {

				$table_coupon .= "<td style='text-align:center;background-color:#F2F2F2'><b>".number_format($total_member)."</b></td>";
			}
...
$oTmp->assign('content_file', 'analytics/top_coupon.htm');
