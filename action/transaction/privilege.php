...
if ($_SESSION['role_action']['privilege_insert']['view'] != 1) {
...
$sql_transaction = 'SELECT
						...
						member_transaction_h.meth_UpdatedDate AS update_date,
						...
						LEFT JOIN mi_user
						ON mi_user.user_id = member_transaction_h.meth_UpdatedBy 

						LEFT JOIN mi_user_type
						ON mi_user.user_type_id = mi_user_type.user_type_id 

						WHERE member_transaction_h.meth_Platform = "Insert"
						'.$where_brand.'
...
if (!$rs_trans) {
	...
	while($axRow = $rs_trans->FetchRow(DBI_ASSOC)) {
		...
		while($view = $rs_view->FetchRow(DBI_ASSOC)) {
			...
			if ($view['type'] == "Privilege") {
				...
				$sql_del = 'SELECT COUNT(mepe_MemberPrivlegeID) 
							FROM member_privilege_trans 
							WHERE meth_MemberTransactionHID="'.$axRow['id_use'].'"
							AND priv_PrivilegeID="'.$view['privilege_id'].'"
							AND mepe_Deleted="T"';

			} elseif ($view['type'] == "Coupon" || $view['type'] == "Birthday Coupon") {
				...
				$sql_del = 'SELECT COUNT(meco_MemberCouponID) 
							FROM member_coupon_trans 
							WHERE meth_MemberTransactionHID="'.$axRow['id_use'].'"
							AND coup_CouponID="'.$view['privilege_id'].'"
							AND meco_Deleted="T"';
				...
				$sql_del = 'SELECT COUNT(meac_MemberActivityID) 
							FROM member_activity_trans 
							WHERE meth_MemberTransactionHID="'.$axRow['id_use'].'"
							AND acti_ActivityID="'.$view['privilege_id'].'"
							AND meac_Deleted="T"';
			}
		...
		while($del = $rs_del->FetchRow(DBI_ASSOC)) {
			...
			if ($del['type'] == "Privilege") {

				$head = "priv";
				$table = "privilege";

			} elseif ($del['type'] == "Coupon" || $del['type'] == "Birthday Coupon") {
			...
			if($del['privilege_image']!=''){

				$privilege_image = '<img src="../../upload/'.$del['privilege_path'].$del['privilege_image'].'" class="image_border" height="50"/>';

			} else {

				$privilege_image = '<img src="../../images/card_privilege.jpg" class="image_border" height="50"/>';
			}


			# REASON

			if ($del['reason_del']=='') { $del['reason_del'] = '-'; }


			# DETAIL

			$privilege_del = '<table style="width:100%">
									<tr>
										<td style="text-align:right;width:60px">
											Name<br>
											Type<br>
			...
			$data_del .= '<tr>
								<td style="text-align:center">'.$privilege_image.'</td>
								<td>'.$privilege_data.'</td>
								<td style="text-align:center">'.$del['reason_del'].'</td>
								<td style="text-align:center">'.DateTime($del['delete_date']).'<hr>'.$del['delete_by'].'</td>
							</tr>';
		}
						...
						<div class="modal-content">
						    <div class="modal-body" align="left">
					            ...
					            <ul id="tapMenu" class="nav nav-tabs">
					                <li class="active">
					                    <a data-toggle="tab" href="#use'.$n.'">
					                    <center><b>Use Data</b></center></a>
					                </li>
					                <li>
					                    <a data-toggle="tab" href="#del'.$n.'">
					                    <center><b>Delete Data</b></center></a>
					                </li>
					            </ul>
					            <div class="tab-content">
					                <div id="use'.$n.'" class="tab-pane active"><br>
									    <table width="100%" class="table table-striped table-bordered myPopup">
									        <thead>
												<tr class="th_table">
													...
													<th style="width:10%;text-align:ceter"><b>No. of Delete</b></th>
												</tr>
											</thead>
											<tbody>
												'.$data_view.'
					                ...
					                <div id="del'.$n.'" class="tab-pane"><br>
									    <table width="100%" class="table table-striped table-bordered myPopup">
									        <thead>
												<tr class="th_table">
													<th style="width:20%;text-align:ceter"><b>Privilege</b></th>
													<th style="text-align:ceter"><b>Detail</b></th>
													<th style="width:20%;text-align:ceter"><b>Reason</b></th>
													<th style="width:10%;text-align:ceter"><b>Deleted</b></th>
												</tr>
											</thead>
											<tbody>';

		if ($data_del) { $view .= $data_del; } 
		else { $view .= '<tr><td colspan="4" style="text-align:ceter">No Data Deleted</td></tr>'; }
		...
		if ($_SESSION['role_action']['privilege_insert']['delete'] == 1 && $axRow['status_del']=='') {		    
			
			$view .= '       <a href="privilege_delete.php?act=edit&id='.$axRow['id_use'].'">
						        <button type="button" class="btn btn-default btn-sm">Select</button></a>';
		}
		...
		$data_table .= '<tr>
							...
							<td style="text-align:center">'.DateTime($axRow['update_date']).'
...
$oTmp->assign('content_file','transaction/privilege.htm');
