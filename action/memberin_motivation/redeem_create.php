...
if( $Act == 'edit' && $id != '' ){
	...
	$rera_HomeAddress = trim_txt($_REQUEST['rera_HomeAddress']);	

	$rera_BankAccount = trim_txt($_REQUEST['rera_BankAccount']);		

	$rera_Email = trim_txt($_REQUEST['rera_Email']);		

	$rera_Mobile = trim_txt($_REQUEST['rera_Mobile']);		
	...
	$sql_ratio .= ',rera_HomeAddress="'.$rera_HomeAddress.'"';	
	$sql_ratio .= ',rera_BankAccount="'.$rera_BankAccount.'"';	
	$sql_ratio .= ',rera_Email="'.$rera_Email.'"';	
	$sql_ratio .= ',rera_Mobile="'.$rera_Mobile.'"';	
	...
	$sql_ratio .= ',rera_RewardQty="'.$rera_RewardQty.'"';
...
$as_reward = dropdownlist_from_table($oDB,'reward','rewa_RewardID','rewa_Name',' bran_BrandID="0"',' ORDER BY rewa_Name ASC');
...
$as_icon = dropdownlist_from_table($oDB,'collection_type','coty_CollectionTypeID','coty_Name',' coty_Type="MemberIn" AND coty_Status="T"');
...
$oTmp->assign('content_file', 'memberin_motivation/redeem_create.htm');
