...
$sql = 'SELECT 
		...
		WHERE hilight_coupon.coup_Type = "Buy"
	...
	while ($axRow = $oRes->FetchRow(DBI_ASSOC)){
		...
		if($axRow['coup_Deleted']=='T'){
			...
			if($axRow['coup_Status']=='Active'){
				...
				if ($_SESSION['role_action']['promotion']['edit'] == 1) {

					$status = '<form id="myForm" method="POST">
						<select class="form-control text-md status_active" name="active_status" onchange="window.location.href=\'buy.php?act=active&id='.$axRow['coup_CouponID'].'\'">
				...
				if ($_SESSION['role_action']['promotion']['edit'] == 1) {
					...
					$status = '<form id="myForm" method="POST">
						<select class="form-control text-md status_pending" name="pending_status" onchange="window.location.href=\'buy.php?act=pending&id='.$axRow['coup_CouponID'].'\'">
		...
		if($axRow['coup_SendEmail']=='T'){
			...
			if ($_SESSION['role_action']['promotion']['edit'] == 1) {
				...
				$send_email = '<form id="myForm" method="POST">
						<select class="form-control text-md status_active" name="send_status" onchange="window.location.href=\'buy.php?act=send&id='.$axRow['coup_CouponID'].'\'">
			...
			if ($_SESSION['role_action']['promotion']['edit'] == 1) {
				...
				$send_email = '<form id="myForm" method="POST">
						<select class="form-control text-md status_inactive" name="no_status" onchange="window.location.href=\'buy.php?act=no_send&id='.$axRow['coup_CouponID'].'\'">
					...
					<div class="modal-dialog" role="document">
						<div class="modal-content">
						    ...
						    <div class="modal-footer">
						    	<a href="buy.php?act=delete&id='.$axRow['coup_CouponID'].'">
		...
		# VIEW
			...
			if ($axRow['coup_Time'] == 'Fix') {
				...
				$time = '<tr>
							<td style="text-align:right" width="45%">Start Time Buy</td>
							<td style="text-align:center" width="5%">:</td>
							<td>'.$axRow['coup_StartTime'].'</td>
						</tr>
						<tr>
							<td style="text-align:right">End Time Buy</td>
				...
				$time = '<tr>
							<td style="text-align:right" width="45%">Buy Time</td>
							<td style="text-align:center" width="5%">:</td>
							<td>All Day</td>
						</tr>';
			}
			...
			if ($axRow['coup_Method'] == 'Dpd') {
				...
				$validate = '<tr>
								<td style="text-align:right" width="45%">Start Buy Date</td>
								<td style="text-align:center" width="5%">:</td>
								<td>'.DateOnly($axRow['coup_StartDate']).'</td>
							</tr>
							<tr>
								<td style="text-align:right" width="45%">End Buy Date</td>
								<td style="text-align:center" width="5%">:</td>
								<td>'.DateOnly($axRow['coup_EndDate']).'</td>
							</tr>';
			}

			if ($axRow['coup_MethodUse'] == 'No') {

				$use_date = '<tr>
								<td style="text-align:right" width="45%">Use Date</td>
								<td style="text-align:center" width="5%">:</td>
				...
				$use_date = '<tr>
								<td style="text-align:right" width="45%">Start Use Date</td>
								<td style="text-align:center" width="5%">:</td>
								<td>'.DateOnly($axRow['coup_StartDateUse']).'</td>
							</tr>
							<tr>
								<td style="text-align:right" width="45%">End Use Date</td>
								<td style="text-align:center" width="5%">:</td>
								<td>'.DateOnly($axRow['coup_EndDateUse']).'</td>
							</tr>';
			}
			...
			if ($axRow['coup_Repetition'] == '') { 
				...
				$axRow['coup_Repetition'] = '<tr>
												<td style="text-align:right" width="45%">Total Limitation</td>
												<td style="text-align:center" width="5%">:</td>
												<td> - </td>
											</tr>';	
			...
			if ($axRow['coup_RepetitionMember'] == '') { 
				...
				$axRow['coup_RepetitionMember'] = '<tr>
													<td style="text-align:right" width="45%">Limitation</td>
													<td style="text-align:center" width="5%">:</td>
													<td> - </td>
												</tr>';	
			...
			if ($axRow['coup_Video'] == '') { $axRow['coup_Video'] = '-';	}

			$percent_off = (($axRow['coup_Cost']-$axRow['coup_Price'])/$axRow['coup_Cost'])*100;
					                ...
					                <div class="tab-content">
					                    <div id="basic'.$axRow['coup_CouponID'].'" class="tab-pane active"><br>
								        	<table width="80%" class="myPopup">
								        		...
								        		<tr height="35px">
								        			<td style="text-align:right"><span class="glyphicon glyphicon-facetime-video" height="25" alt="Video"></span></td>
								        			<td style="text-align:center">:</td>
								        			<td>'.$axRow['coup_Video'].'</td>
								        		</tr>
								        	</table>
					                    </div>
					                    ...
					                    <div id="location'.$axRow['coup_CouponID'].'" class="tab-pane"><br>
					                    	...
					                    	<table style="width:80%;" class="table table-striped table-bordered myPopup">
								        		<thead><tr class="th_table">
				                                    ...
				                                    <td style="text-align:center"><b>Qr Code</b></td>
				                                    <td style="text-align:center"><b>Template</b></td>
				                                </tr></thead>
					                    ...
					                    <div id="financial'.$axRow['coup_CouponID'].'" class="tab-pane"><br>
					                    	<table width="80%" class="myPopup">
								        		<tr>
								        			<td style="text-align:right" width="45%">Sale Price</td>
								        			<td style="text-align:center" width="5%">:</td>
								        			<td>'.number_format($axRow['coup_Price'],2).' ฿. ('.number_format($percent_off).'% Off)</td>
								        		</tr>
								        		<tr>
								        			<td style="text-align:right">Original Price</td>
								        			<td style="text-align:center">:</td>
								        			<td>'.number_format($axRow['coup_Cost'],2).' ฿.</td>
								        		</tr>
								        	</table>
					                    </div>
		...
		$view .= '					</div>
			...
			if ($_SESSION['role_action']['promotion']['edit'] == 1) {		    
				...
				$view .= '       <a href="buy_create.php?act=edit&id='.$axRow['coup_CouponID'].'">
...
$oTmp->assign('content_file', 'earn_attention/buy.htm');
...
<script type="text/javascript">
	...
	function VarietySelect(variety_id,coupon_id) {
      ...
      window.location.href="buy.php?act=variety_category&id="+coupon_id+"&variety_id="+variety_id;
	}
	
	function DisplaySelect(display_id,coupon_id) {

      window.location.href="buy.php?act=display_data&id="+coupon_id+"&display_id="+display_id;
	}
