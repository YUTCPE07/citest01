...
$time_pic = date("Ymd");
$path_qr_code_upload = $_SESSION['path_upload_highlight'];
$path_upload_earn_attention = $_SESSION['path_upload_earn_attention'];
...
#######################################
	...
	if( $Act == 'edit' && $id != '' ) {
		...
		$sql .= 'SELECT * FROM hilight_coupon WHERE coup_CouponID = "'.$id.'" '.$where_brand.' ';
		$oRes = $oDB->Query($sql);
		...
		if($id){
			...
			$qrcode_privileges_text = "QCH-".str_pad($id,4,"0",STR_PAD_LEFT)."-"
			...
			$qr_image = ImageCreateFromPng($file_full_path);
			$qr_logo = imagecreatefromJpeg('../../images/qrcode_logo.jpg');
							 
			ImageCopyMerge($qr_image, $qr_logo, 98, 98, 0, 0, 94, 94, 100);
...
$oTmp->assign('content_file', 'earn_attention/use_create.htm');
