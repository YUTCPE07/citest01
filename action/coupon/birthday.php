...
if ($_SESSION['role_action']['hbd_coupon']['view'] != 1) {
...
if ($_SESSION['user_type_id_ses']>1 ) {
	...
	$where_brand = ' AND coupon.bran_BrandID = "'.$_SESSION['user_brand_id'].'" AND coup_Deleted=""';
}
...
if($Act == 'approve' && $id != '') {
	...
	$reportName = 'Birthday Coupon Report';
	...
	while ($axRow = $oRes->FetchRow(DBI_ASSOC)){
		...
		if ($axRow['coup_Approve']=='' && $axRow['coup_Image']=='') {
            ...
            $button_unapprove = '<a style="cursor:pointer" href="birthday.php?act=approve&approve=unapprove&id='.$axRow['coup_CouponID'].'">
			...
			$button_approve = '<a style="cursor:pointer" href="birthday.php?act=approve&approve=approve&id='.$axRow['coup_CouponID'].'">
                                <button type="button" class="btn btn-default btn-sm">
		...
		if($axRow['coup_Deleted']=='T'){
			...
			if($axRow['coup_Status']=='Active'){
				...
				if ($_SESSION['role_action']['hbd_coupon']['edit'] == 1) {
					...
					<div class="modal-dialog" role="document">
						<div class="modal-content">
						    ...
						    <div class="modal-footer">
						    	<a href="birthday.php?act=delete&id='.$axRow['coup_CouponID'].'">
		...
		# VIEW
			...
			if ($axRow['coup_Payment'] == '') { 

				$axRow['coup_Payment'] = '<span class="glyphicon glyphicon-unchecked"></span> Credit Card';	

			} else if ($axRow['coup_Payment'] == 'CreditCard') {

				$axRow['coup_Payment'] = '<span class="glyphicon glyphicon-check"></span> Credit Card';	}
			...
			if ($axRow['prod_ProductID'] == 0) {	$product_name = '-';	$product_img = '';	}
				...
				$path_img = 'SELECT path_image FROM mi_products WHERE products_id="'.$axRow['prod_ProductID'].'"';

				$path_img = $oDB->QueryOne($path_img);

				$product_img = '<img src="../../upload/'.$path_img.$product_img.'" width="150" height="150"/>';
			}
		...
		$view .= '	                    <li>
			...
			if ($_SESSION['role_action']['hbd_coupon']['edit'] == 1) {		    
				...
				$view .= '       <a href="birthday_create.php?act=edit&id='.$axRow['coup_CouponID'].'">
		...
		if ($_SESSION['role_action']['hbd_coupon']['delete'] == 1) {
...
$oTmp->assign('content_file', 'coupon/birthday.htm');
